# Chart13 
### description
wrong argument: `java.lang.IllegalArgumentException: Range(double, double): require lower (0.0) <= upper (-2.3000000000000007).`
second arg should <= first arg

### developer's patch
```
diff -r c13b/source/org/jfree/chart/block/BorderArrangement.java c13f/source/org/jfree/chart/block/BorderArrangement.java
455c455
...
```
### jKali
incorrect. `w[2]` is 0.0 by default. The following code snippet (the deleted if block) assigns it a positive number to `w[2]`. jKali deletes this block to keep `w[2]` 0 so that `new Range(0.0, Math.max(constraint.getWidth() - w[2], 0.0))` will not throw the exception.
```
